...
//reads training and test file puts into arrays.
void read_file(void)
{
	FILE * fp;
	char * line = NULL;
...
//outputs a digit in the console. 
void output_digit(unsigned int Xi[])
{
	int Line =sqrt(FEATURES);
	for (int y = 0; y < Line; y++) {
		for (int x = 0; x < Line; x++) {
			int chunk_nr = (x + y*Line) / INT_SIZE;
			int chunk_pos = (x + y*Line) % INT_SIZE;
...
{	
	...
	for (int i = 0; i < EPOCHS; i++) {
		...
		fprintf(fo, "%.2f ," , 100*test_accuracy);
